...
const dicelog = [0]; //instead of checking for undefined, put a 0 as first element (no one can roll a 0)
console.log(dicelog);
...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (dicelog[0] === 0 || args[1] === 'throw') {
      embedDice.setFooter('Note: Try adding higher/lower');
    };

    if (typeof dicelog[0] !== 'undefined' && guesses.includes(args[1])) {
      if ((dice > dicelog[0] && (args[1] === guesses[0] || args[1] === guesses[1])) || (dice < dicelog[0] && (args[1] === guesses[2] || args[1] === guesses[3]))) {
